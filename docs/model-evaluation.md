...
当前状态：本地 200 条模型协议 dry-run 已通过；真实模型 200 条评测尚未执行，已有启动器负责密钥预检与脱敏报告写入，仍需在本机配置密钥后运行。根据本轮用户决定，真实模型评测暂缓且不计为通过；本轮候选发布不以它为阻断条件，但正式发布仍需补做。本轮 Windows 启停脚本验收和 S3 来源扩充均不替代真实模型评测。
...
本轮若按用户决定暂缓三项外部证据，可运行：

```powershell
pnpm release:candidate
```

该命令只验证工程质量，并把真实模型评测显示为延期/未通过；它不生成虚假的模型通过报告。
