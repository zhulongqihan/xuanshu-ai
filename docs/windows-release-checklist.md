...
当前状态：隔离目录安装与运行基线验收已完成，并在后续提交上补做了生产服务浏览器链路验收；来源注册、参考案例计数和真实模型评测启动器独立维护。另一台真实 Windows 机器的桌面快捷方式和升级验收仍待执行；本轮用户已明确暂缓该门禁，因此它仍是“未通过”而不是“已验收”，不阻止候选发布检查。阶段总状态以[质量基线与路线图](./quality-and-roadmap.md)为准。
...
未完成以上另一台机器步骤前，发布状态只能写成“候选发布”，不能写成“已完成 Windows 发布验收”。本轮可用 `pnpm release:candidate` 完成本地工程质量收口，但它不会替代这份清单。

完成以上步骤并保留验收记录后，在目标环境运行：

```powershell
pnpm release:check -- -SecondWindowsVerified
```

该参数只表示操作者已完成清单，不替代清单本身、日志和页面验收证据。
