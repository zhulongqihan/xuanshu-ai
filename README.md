...
当前已进入首版候选发布收口。真实模型评测、独立参考案例复核，以及另一台真实 Windows 机器的桌面发布验收仍未执行；根据本轮用户决定，这三项正式证据暂缓且不计为通过，不阻止候选发布检查。公开 S3 交叉验证来源已登记，但不替代人工黄金案例。真实模型评测已有脱敏报告启动器，后续配置本机密钥后仍可补做。详细状态以[质量基线与路线图](./docs/quality-and-roadmap.md)为准。
...
它会运行工程质量检查，并检查真实模型评测、脱敏参考案例闸门和第二台 Windows 验收状态；
缺少外部证据时会明确返回阻断，不会用跳过或模拟结果冒充正式发布完成。

本轮按用户决定暂缓三项外部证据时，可运行候选发布检查：

```powershell
pnpm release:candidate
```

该命令只放行工程质量检查，并在结果中明确列出真实模型评测、独立参考案例和第二台 Windows
验收为“延期/未通过”。它表示当前代码已完成候选发布范围的收口，不表示正式发布或黄金集验收通过。

完成另一台 Windows 清单后，需显式确认该人工门禁并使用正式闸门：

```powershell
pnpm release:check -- -SecondWindowsVerified
```
